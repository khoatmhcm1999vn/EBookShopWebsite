...
export const submitResendToken = email => async (dispatch, getState) => {
  try {
    ...
    console.log(data)
    dispatch({
      type: userTypes.RESEND_TOKEN_SUCCESS
    })
    ...
    // swal("Error!", res.data.message, "error");
    dispatch({
      type: userTypes.RESEND_TOKEN_FAIL
    })
    return
  }
}
export const resetResendToken = () => ({
  type: userTypes.RESET_RESEND_TOKEN
})
