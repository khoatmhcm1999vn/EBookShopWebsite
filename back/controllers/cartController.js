...
exports.addItemToCart = (req, res) => {
  Cart.findOne({ user: req.profile._id }).exec((error, cart) => {
    ...
    if (cart) {
      ...
      req.body.cartItems.forEach((cartItem) => {
        ...
        if (item) {
          condition = { user: req.profile._id, "cartItems.product": product };
          update = {
            $set: {
              "cartItems.$": cartItem,
            },
          ...
          condition = { user: req.profile._id };
          update = {
            $push: {
              cartItems: cartItem,
            },
...
exports.getCartItems = (req, res) => {
  ...
  Cart.findOne({ user: req.profile._id })
    ...
    .populate(
      "cartItems.product",
      "_id name description price quantity sold imageUrl"
    )
    .exec((error, cart) => {
      if (error) return res.status(400).json({ error });
      if (cart) {
